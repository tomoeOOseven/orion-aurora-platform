...
async function findBestNearbyPoint(lat, lon, ovationPoints, radiusKm = 200, options = {}) {
  ...
  for (const pt of (ovationPoints || [])) {
    ...
    if (pt.aurora < minAuroraPct) continue;
    candidates.push({
      lat: pt.lat,
      lon: pt.lon,
      aurora: pt.aurora,
  ...
  if (candidates.length === 0) return { found: false, reason: `No points with aurora >= ${minAuroraPct}% within radius` };

  // Keep only nearest candidates before cloud/Bortle checks to avoid huge external I/O in global scans.
  candidates.sort((a, b) => a.distanceKm - b.distanceKm);
  const candidatePool = candidates.slice(0, maxNearestCandidates);

  // Step 2: batch cloud fetch for unique cache cells
  const uniqueCloudKeys = [...new Set(candidatePool.map(c => cloudKey(c.lat, c.lon)))];
  await Promise.all(uniqueCloudKeys.map(k => {
    const [klat, klon] = k.split(',').map(Number);
    return fetchCloudCover(klat, klon);
  }));

  // Step 3: filter cloud by configured threshold from cache
  const clearCandidates = candidatePool.filter(c => {
    const hit = cloudCache.get(cloudKey(c.lat, c.lon));
    return hit ? hit.totalCloud <= maxCloudPct : false;
  }).map(c => ({
    ...c,
    cloudCoverPct: cloudCache.get(cloudKey(c.lat, c.lon)).totalCloud,
  }));

  if (clearCandidates.length === 0) {
    return { found: false, reason: `No locations with aurora >= ${minAuroraPct}% and cloud <= ${maxCloudPct}% within radius` };
  }

  // Step 4: prioritize nearest points first for practical GPS navigation
  clearCandidates.sort((a, b) => a.distanceKm - b.distanceKm);
  ...
  if (!chosen) {
    return {
      found: false,
      reason: `No locations meeting constraints (aurora >= ${minAuroraPct}%, cloud <= ${maxCloudPct}%, Bortle <= ${maxBortle}) within radius`,
    };
  }

  const cloudScore = Math.max(0, 100 - chosen.cloudCoverPct);
  const { darkness } = darknessFromBortle(chosen.lat, chosen.lon, chosen.bortle);
  const score = Math.min(100, Math.round(
    chosen.aurora * WEIGHTS.aurora + cloudScore * WEIGHTS.cloud + darkness * WEIGHTS.darkness
  ));
