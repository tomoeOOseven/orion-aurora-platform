...
function computeAlertBadge(substormAlert, noaaAlerts) {
  const messages = (noaaAlerts || []).map((a) => String(a?.message || '')).join(' | ');
  const hasWarnings = /\bwarning\b|\bK0[5-9]W\b|\bK1\dW\b/i.test(messages);
  const hasWatch = /\bwatch\b/i.test(messages);
  ...
  if ((noaaAlerts || []).length > 0) {
    return { label: 'ACTIVE', color: 'var(--aurora-teal)', bg: 'rgba(0,188,212,0.12)', border: 'rgba(0,188,212,0.3)' };
  }
  return { label: 'CLEAR', color: 'var(--success)', bg: 'rgba(0,230,118,0.12)', border: 'rgba(0,230,118,0.3)' };
}
